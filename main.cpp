...
int main()
{
    ...
    try
    {
        ...
        SSL_set_tlsext_host_name(secureSocket.native_handle(),
                                 input_domain.c_str()); // SNI
        ...
        while (
            asio::read(secureSocket, response, asio::transfer_at_least(1), error))
        {
            htmlBodyStream << &response;
        }
        ...
        htmlParser::tokenizePrint(htmlBody);
    }
