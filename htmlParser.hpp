...
private:
    ...
    static std::string decodeEntities(std::string text) {
        ...
        while ((pos = text.find("&#", pos)) != std::string::npos) {
            ...
            if (end_pos != std::string::npos) {
                try {
                    std::string code_str = text.substr(pos + 2, end_pos - pos - 2);
                    int codepoint = 0;
                    
                    if (!code_str.empty() && (code_str[0] == 'x' || code_str[0] == 'X')) {
                        codepoint = std::stoi(code_str.substr(1), nullptr, 16);
                    } else {
                        codepoint = std::stoi(code_str);
                    }
                    
                    std::string utf8_char;
                    if (codepoint <= 0x7F) utf8_char += static_cast<char>(codepoint);
    ...
    static std::string getAttribute(const std::string& tagContent, const std::string& attr) {
        ...
        if (pos != std::string::npos) {
            ...
            if (end != std::string::npos) return tagContent.substr(start, end - start);
        }
        return "";
    }
  ...
  static std::vector<DOMNode> parseDOM(const std::string &html) {
    ...
    while (cursor < html.length()) {
      ...
      if (openBracket > cursor) {
        ...
        if (in_style_tag) raw_css += original;
      ...
      if (!tagContent.empty()) {
        if (tagContent[0] == '/') {
          ...
          if (tagName == "script" || tagName == "style") { ignore_text = false; in_style_tag = false; }
          if (tagName == "form") current_form_action = "";
          current_tag = "span"; current_url = ""; 
        } else {
          size_t spacePos = tagContent.find(' ');
          std::string tagName = tagContent.substr(0, spacePos);
          
          if (tagName == "script") ignore_text = true;
          else if (tagName == "style") { ignore_text = true; in_style_tag = true; }
          else {
              current_tag = tagName; current_url = ""; 

              if (tagName == "a") current_url = getAttribute(tagContent, "href");
              else if (tagName == "img") {
                  std::string img_url = getAttribute(tagContent, "src");
                  if (!img_url.empty()) dom_tree.push_back({"img", "[IMG]", "", true, img_url, {}, "", 0, false, "", "", false, ""});
              }
    ...
    while (css_cursor < raw_css.length()) {
        ...
        std::string parsed_color = ""; int parsed_size = 0;
        std::stringstream prop_stream(properties); std::string prop;
        while (std::getline(prop_stream, prop, ';')) {
            ...
            if (colon != std::string::npos) {
                ...
                if (key == "font-size") { try { parsed_size = std::stoi(val); } catch(...) {} }
            }
        }
        std::stringstream sel_stream(selectors); std::string sel;
    ...
    for (auto& node : dom_tree) {
        if (styles.count(node.tag)) {
            node.css_color = styles[node.tag].first; node.css_font_size = styles[node.tag].second;
        }
    }
    return dom_tree;
  }
